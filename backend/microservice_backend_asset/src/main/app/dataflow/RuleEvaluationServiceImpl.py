...
class RuleEvaluationService(object):
    ...
    def rules_evaluation(self, user_id, rules):
        rule_list = []
        for rule_id in rules:
            output = self.rule_evaluation(user_id, rule_id)
            if output == "true":
                rule_list.append(rule_id)
        for rule_id in rule_list:
            app.consequent_evaluation_service.consequent_evaluation(user_id, rule_id)
